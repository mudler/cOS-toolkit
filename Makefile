...
iso:
	...
	yq w -i $(ISO_SPEC).remote 'repositories[0].name' 'cOS'
	yq w -i $(ISO_SPEC).remote 'repositories[0].enable' true
	yq w -i $(ISO_SPEC).remote 'repositories[0].type' 'docker'
	yq w -i $(ISO_SPEC).remote 'repositories[0].urls[0]' $(FINAL_REPO)
